...
const DashboardStatCard = ({ value, label, helperText, isEmphasized = false }: DashboardStatCardProps) => {
  const borderClassName = isEmphasized ? 'border-ember-300/45 bg-ember-500/10' : 'border-white/12 bg-[#131112]'
  const valueClassName = isEmphasized ? 'text-ember-200' : 'text-white'

  return (
    <article className={`rounded-xl border p-4 ${borderClassName}`}>
      <p className={`font-[family-name:var(--font-heading)] text-4xl font-semibold italic leading-none ${valueClassName}`}>{value}</p>
